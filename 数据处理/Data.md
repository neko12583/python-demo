...
> 注意使用open()的时候，不能有from os import *,因为os模块也有一个open函数会覆盖内建函数open
...
> DDL:(事物操作语句,默认直接提交,不能通过rollback恢复) create drop alter
>
> DML:(数据操控语句,) select  delete update insert
>
> DCL:(数据控制语句) commit rollback grant revoke
...
```
insert into 要插入数据的库.表 (字段) select 字段 from 库.表　where 条件；
```
...
select * from class where between 8 and 10;
...
datetime()在不给值时默认返回值为NULL，而timestamp()在不给值时会默认返回系统的当前时间。
...
* 模糊查询和正则查询
  ...
  1. 模糊查询
     ...
     > 表达式需完全符合匹配结果
	   ...
	   LIKE用于在where子句中进行模糊查询，SQL LIKE 子句中使用百分号` %`来表示任意0个或多个字符，下划线`_`表示任意一个字符，需要加引号。
...
* as 用法
  ...
  > 派生表：将子查询做为一个表来处理
   ...
   ```sql
   select name as 姓名,age as 年龄 from class_1;   # 对原表/字段无影响
...
* 排序
  ...
  使用 ORDER BY 子句将查询数据排序后再返回数据：    #默认升序
	...
	select * from class order by score desc,age;
		# 按照score降序排列，score一样时，按照age默认升序排列
	select * from class order by age,score;
		# 按照age默认升序排列，age一样时，按照score默认升序排列
	```
...
*  联合查询
	...
	默认UNION后为 DISTINCT表示删除结果集中重复的数据。如果使用ALL则返回所有结果集，包含重复数据。(且关系)
	
	> 当两个查询结果列数不一致时会报错

```sql
select * from class where sex='m' UNION ALL select * from class where age > 9;
```
* 子查询
...
> 注意： 此时select 使用了聚合函数后只能查找聚合函数结果，无法查找其他字段，除非配合group by
>
> 
>
> 在子查询中使用了聚合函数，在父查询中想使用聚合函数结果，需给聚合函数结果起别名
>
> 
>
> ROUND(column_name-变量名,decimals-返回的小数位数)
...
>  注意： 使用分组时select 后的字段只能为group by分组的字段和聚合函数，不能包含其他内容。group by也可以同时依照多个字段分组，如group by A，B 此时必须A,B两个字段值均相同才算一组。
...
```sql
# 创建部门表
CREATE TABLE dept (d_id int PRIMARY KEY auto_increment,d_name VARCHAR(20) not null);
```
...
- foreign key 外键的定义语法：
  ...
  ```sql
  [CONSTRAINT (# constraint 约束) symbol(# 外键名)] FOREIGN KEY（外键字段） REFERENCES (# references) tbl_name (主表主键)
  
  约束选项：事件触发限制: on delete和on update ,可设参数cascade(跟随外键改动), restrict(限制外表中的外键改动),set Null(设空值）,set Default（设默认值）,[默认]no action
...
> 部门表和员工表，在员工表建立外键，主表为部门表，从表为员工表
...
- 左连接  : 左表为主表，显示右表中与左表匹配的项，若右表没有对应值，则为null
...
- 右连接 ：右表为主表，显示左表中与右表匹配的项，若右表没有对应值，则为null
...
> 对查询语句得到的结果的映射
>
> 是一张虚拟表，不额外占用数据库物理空间，可以理解为一种快捷方式
>
> 一直存在库中
>
> 不允许出现重复字段
...
CREATE VIEW： 创建视图
OR REPLACE : (replace)可选，如果添加原来有同名视图的情况下会覆盖掉原有视图
view_name ： 视图名称
...
* 隔离级别
   ...
   - 可重复读：repeatable read
     ...
     > 
     >
     > 幻象读：是指两次执行同一条 select 语句会出现不同的结果，第二次读会增加一数据行，并没有说这两次执行是在同一个事务中。一般情况下，幻象读应该正是我们所需要的。但有时候却不是，如果打开的游标，在对游标进行操作时，并不希望新增的记录加到游标命中的数据集中来。
     >
     > **事务A读了一次数据之后事务B插入了一个数据，事务A再读时还是之前所读到的东西（可重复读）但是实际上里面的数据已经多了一条**。这时如果再创建多的这一条数据（因为事务A不知道这数据已经存在了）或者修改与之相关的数据，就会产生错误了。
...
* **常用存储引擎特点** 
  ...
  **MyISAM**
  ...
  ```sql
  1. 只支持表级锁,在锁定期间，其它进程无法对该表进行写操作。如果你是写锁，则其它进程则读也不允许
  2.  表字段和索引分开存储
        	 1. 表名.frm ：表结构
...
垂直拆分 ： 表中列太多，分为多个表，每个表是其中的几个列。将常查询的放到一起， 或者text类型字段放到另一个表
...
> 不是sql命令，需在命令行执行，由MySQL系统提供，只能作用与MySQL
...
1. 用root用户登录mysql
   mysql -u root -p
2. 添加用户 % 表示自动选择可用IP
   CREATE USER 'username'@'host（填地址或%）' IDENTIFIED BY 'password';
3. 权限管理

   # 增加权限
   grant 权限列表 on 库.表 to "用户名"@"%" identified by "密码" with grant option;
   > with grant option 是为了可以让用户再赋予用户权限
   # 删除权限
...
3. 游标方法: cur.execute("insert ....")/cur.executemany("insert ....")   

   #能接收返回值，但是只能得到返回值的数量，不能获得内容

4. 提交到数据库或者获取数据 : db.commit()/cur.fetchall()/cur.fetchone() /cur.fetchmany(number)

   #cur可接收select返回值，为可迭代对象
