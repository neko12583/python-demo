...
* 打开关闭终端方法
  * 点击图形界面终端图标，通过ctrl+alt +t （打开一个新终端页面） ,shift+ctrl + t（在同一窗口再开一个终端）  , shift+ctrl+n（另弹出一终端） 都可以快速打开一个终端。
...
| 08 | mv | 移动一个文件 |
| 09 | clear | 清屏 |
| 10 | ipconfig | 显示当前电脑的TCP/IP网络配置值命令 |
...
```
Linux常见的bai文件类型有：普通文件、目录文件、字符设du备文件和块zhi设备文件、符号链接文件等，其中：dao
...
cat 命令功能强大，特别是与重定向符配合，可实现许多功能。 cat >文件名：新建文件。 cat 文件名：查看文件内容。 cat 文件1 文件2 >文件3：合并文件1和文件2，产生新文件。 cat >>文件名：向文件添加内容。 但是注意输出文件不能同时作为输入文件。
